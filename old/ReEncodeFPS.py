...
class FFmpegConverter(QMainWindow):
    def __init__(self):
        ...
        self.output_input = QLineEdit("Done")
    ...
    def start_conversion(self):
        ...
        output_folder = self.output_input.text() or "Done"
        ...
        for file_path in self.files:
            try:
                ...
                if os.path.exists(output_path):
                    self.status_label.setText(f"Skipping {filename} - already exists")
                    continue
                ...
                if result.returncode == 0:
                    success_count += 1
                    self.status_label.setText(f"Converted {filename}")
                else:
                    self.status_label.setText(f"Error converting {filename}: {result.stderr}")
                
            except Exception as e:
                self.status_label.setText(f"Error processing {filename}: {str(e)}")
        
        self.status_label.setText(f"Conversion complete. {success_count}/{len(self.files)} files processed successfully.")
